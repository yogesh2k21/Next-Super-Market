...
const MyOrders = ({user,setOrders,orders}) => {
  ...
  useEffect(async () => {
    ...
    // if(localStorage.getItem("orders")){
      const response = await fetch(`${process.env.NEXT_PUBLIC_MY_BACK_HOST}/product/getMyOrders/`, {
        method: "GET",
        headers: {
      ...
      <div class="container min-h-screen">
        <div class="flex shadow-2xl rounded-2xl m-5">
          <div class="w-full bg-white m-10 p-10 rounded-3xl">
            ...
            {Object.keys(orders).length===0 || (<div class="flex mt-10 mb-5">
            ...
            {Object.keys(orders).slice(0).reverse().map((item) => {  //slice(0).reverse() is used to print the array in reverese order because in localstorage it store keys in sorted ordered.
              return (
